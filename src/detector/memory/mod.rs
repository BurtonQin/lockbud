...
use rustc_middle::mir::{BasicBlock, Body, Location, Place, Terminator, TerminatorKind};
...
impl<'tcx> InvalidFreeDetector<'tcx> {
    ...
    pub fn detect(
        ...
        let manual_drops = self.collect_manual_drop(callgraph);
        ...
        for (loc, api, _callee_id) in callsites {
            ...
            match api {
                ...
                UninitApi::MaybeUninit => {
                    if let Some((dest, _)) = self.dest_args0(body, loc) {
                        maybe_uninits.push((loc, dest));
                    }
                }
                UninitApi::MaybeUninitWrite | UninitApi::PtrWrite => {
                    if let Some((_, Some(place0))) = self.dest_args0(body, loc) {
                        writes.push((loc, place0));
                    }
                }
                UninitApi::AssumeInit => {
                    if let Some((dest, Some(place0))) = self.dest_args0(body, loc) {
    ...
    fn collect_manual_drop(
        ...
        for (callee_id, node) in callgraph.graph.node_references() {
            ...
            let path = self
                .tcx
                .def_path_str_with_substs(instance.def_id(), instance.substs);
            ...
            for caller_id in caller_ids {
                ...
                for loc in callsites {
                    ...
                    let places0 = match self.dest_args0(body, loc) {
