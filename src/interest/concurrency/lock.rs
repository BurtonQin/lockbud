...
use rustc_middle::mir::visit::{Visitor, PlaceContext, NonUseContext, NonMutatingUseContext, MutatingUseContext};
use rustc_middle::mir::{Body, Location, Local};
...
impl<'tcx> LockGuardTy<'tcx> {
	pub fn from_local_ty(local_ty: ty::Ty<'tcx>, tcx: TyCtxt<'tcx>) -> Option<Self> {
		...
		match local_ty.kind() {
			ty::TyKind::Adt(adt_def, substs) => {
				...
				if path.starts_with("std::sync::MutexGuard<") || path.starts_with("sync::mutex::MutexGuard<") {
					return Some(LockGuardTy::StdMutex(substs.types().next().unwrap()));
				} else if path.starts_with("lock_api::mutex::MutexGuard<") || path.starts_with("parking_lot::lock_api::MutexGuard<") {
				...
				} else if path.starts_with("lock_api::rwlock::RwLockReadGuard<") || path.starts_with("parking_lot::lock_api::RwLockReadGuard<") {
					return Some(LockGuardTy::ParkingLotRead(substs.types().nth(1).unwrap()));
				} else if path.starts_with("lock_api::rwlock::RwLockWriteGuard<") || path.starts_with("parking_lot::lock_api::RwLockWriteGuard<") {
	...
	pub fn deadlock_with(&self, other: &Self) -> bool {
		...
		match (self, other)	{
			...
			(SpinRead(a), SpinWrite(b)) if a == b => true,
			_ => false,
		}
	}
	...
	pub fn may_deadlock_with(&self, other: &Self) -> bool {
		...
		match (self, other)	{
			...
			(ParkingLotRead(a), ParkingLotRead(b)) if a == b => true,
			_ => false,
		}
	}
}
...
impl<'a, 'b, 'tcx> LockGuardCollector<'a, 'b, 'tcx> {
	...
	pub fn analyze(&mut self) {
		for (local, local_decl) in self.body.local_decls.iter_enumerated() {
			let local_ty = self.instance.subst_mir_and_normalize_erasing_regions(self.tcx, self.param_env, local_decl.ty);
